-- Fix the sync queue population - use a safer approach without ON CONFLICT
-- First check if the queue is empty to avoid duplicates
DO $$
BEGIN
  -- Only populate if queue is empty
  IF NOT EXISTS (SELECT 1 FROM public.google_places_sync_queue LIMIT 1) THEN
    ...
    RAISE NOTICE 'Populated sync queue with % stores', (SELECT COUNT(*) FROM public.google_places_sync_queue);
  ELSE
    RAISE NOTICE 'Sync queue already populated with % stores', (SELECT COUNT(*) FROM public.google_places_sync_queue);
  END IF;
END $$;
